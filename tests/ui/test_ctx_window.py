...
def _load_with_model(page: Page, base_url: str, model_name: str, context_length: int) -> None:
    ...
    page.wait_for_selector(
        f'#model-select option[value="{model_name}"]', state="attached", timeout=5000
    )
    ...
    page.wait_for_function(
        "document.getElementById('ctx-select-row').style.display === 'flex'", timeout=3000
    )


def _ctx_option_texts(page: Page) -> list[str]:
    """Return all non-empty ctx-select option text values."""
    return [t.strip() for t in page.locator("#ctx-select option").all_text_contents() if t.strip()]
...
class TestCtxLabelRendering:
    ...
    def test_256k_model_shows_256k_label(self, page: Page, static_server: str) -> None:
        ...
        assert any(
            "256K tokens (model max)" in o for o in options
        ), f"Expected '256K tokens (model max)' in ctx options, got: {options}"
        assert not any(
            o == "128K tokens (model max)" for o in options
    ...
    def test_256k_model_includes_smaller_options(self, page: Page, static_server: str) -> None:
        ...
        for expected in [
            ...
            assert any(
                expected in o for o in options
    ...
    def test_1m_model_shows_1m_label(self, page: Page, static_server: str) -> None:
        ...
        assert any(
            "1M tokens (model max)" in o for o in options
    ...
    def test_512k_model_shows_512k_label(self, page: Page, static_server: str) -> None:
        ...
        assert any(
            "512K tokens (model max)" in o for o in options
    ...
    def test_4k_model_shows_no_larger_options(self, page: Page, static_server: str) -> None:
        ...
        for unexpected in ["8K tokens", "16K tokens", "32K tokens", "64K tokens", "128K tokens"]:
            assert not any(
                unexpected in o for o in options
            ), f"'{unexpected}' should not appear for a 4K model, got: {options}"
        assert any(
            "4K tokens (model max)" in o for o in options
...
class TestCtxRowVisibility:
    ...
    def test_ctx_row_hidden_when_no_model_selected(self, page: Page, static_server: str) -> None:
        ...
        displayed = page.evaluate(
            "getComputedStyle(document.getElementById('ctx-select-row')).display"
        )
        assert (
            displayed == "none"
        ), f"ctx-select-row should be hidden with no model, got display={displayed!r}"

    def test_ctx_row_visible_after_model_selected(self, page: Page, static_server: str) -> None:
    ...
    def test_ctx_row_hidden_for_model_without_context_length(
        ...
        page.goto(f"{static_server}/index.html")
        page.wait_for_selector(
            '#model-select option[value="unknown-model:latest"]', state="attached", timeout=5000
        )
